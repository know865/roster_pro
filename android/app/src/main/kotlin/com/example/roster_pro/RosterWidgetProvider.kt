...
class RosterWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        private fun getValueAsInt(sp: SharedPreferences, key: String, def: Int): Int {
            return try { val v = sp.all[key]; when (v) { null -> def; is Int -> v; is Long -> v.toInt(); is Double -> v.toLong().toInt(); is Float -> v.toInt(); is String -> v.toDoubleOrNull()?.toLong()?.toInt() ?: v.toIntOrNull() ?: def; else -> def } } catch (e: Exception) { def }
        }
        ...
        fun updateAppWidget(context: Context, appWidgetManager: AppWidgetManager, appWidgetId: Int) {
            ...
            try {
                ...
                if (textColor == 0) textColor = 0xFF000000.toInt()
