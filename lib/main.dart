...
class RosterApp extends StatelessWidget{
  ...
  Widget build(BuildContext context){
    return MaterialApp(title:'Roster Pro v6.22',theme:ThemeData(useMaterial3:true,colorSchemeSeed:Colors.deepPurple),localizationsDelegates: const [GlobalMaterialLocalizations.delegate, GlobalWidgetsLocalizations.delegate, GlobalCupertinoLocalizations.delegate],supportedLocales: const [Locale('zh','HK'), Locale('en')],home:const MainPage());
  }
}
...
class MainPageState extends State<MainPage>{
  ...
  Map<String,ShiftDef> defs={
    ...
    '早收':ShiftDef('早收','早收長代號不限制',8,Colors.orange,start:"07:00",end:"15:30"),
  };
  ...
  Widget calTab(){
    ...
    return Column(children:[
      ...
      Expanded(child:GridView.builder(padding:const EdgeInsets.symmetric(horizontal:6),gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:7,childAspectRatio:0.78,mainAxisSpacing:6,crossAxisSpacing:6),itemCount:42,itemBuilder:(ctx,idx){
        ...
        Color bg; if(!inMonth){bg=const Color(0xFFF5F5F0);} else if(isSelected){bg=Colors.white;} else if(def!=null){bg=Color.lerp(Colors.white,def.color,0.15)??const Color(0xFFFFF0D0);} else {bg=const Color(0xFFFFF0D0);} if(code=='O'){bg=const Color(0xFFE0F2E9);}
      ...
      Container(padding:const EdgeInsets.fromLTRB(16,12,16,12),decoration:BoxDecoration(color:Colors.white,border:Border(top:BorderSide(color:Colors.grey.shade200))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        if(selDef!=null) Text('${selDef.label} ${selDef.start}-${selDef.end} 工時${selDef.hours}h 津貼\$${selDef.allowance} OT${selDef.ot}h',style:TextStyle(color:Colors.grey.shade600,fontSize:13)),
        const SizedBox(height:8),
        Container(padding:const EdgeInsets.all(12),decoration:BoxDecoration(border:Border.all(color:Colors.blue.shade300),borderRadius:BorderRadius.circular(16)),child:Text(rosterNote[selKey]??'哈哈',style:const TextStyle(fontSize:14))),
      ])),
    ...
    showModalBottomSheet(context:context,isScrollControlled:true,builder:(ctx){
      return StatefulBuilder(builder:(ctx2,setM){
        return Padding(padding:EdgeInsets.only(bottom:MediaQuery.of(ctx2).viewInsets.bottom),child:Container(padding:const EdgeInsets.all(16),child:Column(mainAxisSize:MainAxisSize.min,children:[
          ...
          Wrap(spacing:6,children:defs.keys.map((c){var d=defs[c]!; return ChoiceChip(label:Text(c),selected:selected==c,selectedColor:d.color.withOpacity(0.4),onSelected:(_){setM((){selected=c;});});}).toList()..add(ChoiceChip(label:const Text('清除'),selected:false,onSelected:(_){setM((){selected='__clear';});}))),
          Row(children:[
          ...
          Row(children:[
            ...
            Expanded(child:FilledButton(onPressed:(){
              setState((){
                ...
                else if(selected!=''){roster[k]=selected; rosterNote[k]=noteCtrl.text; double ex=double.tryParse(otCtrl.text)??0; if(ex>0) rosterOT[k]=ex; selectedDay=day;}
              });
  ...
  Widget patternTab(){
    return Column(children:[
      ...
      Expanded(child:ListView.builder(itemCount:pattern.length,itemBuilder:(ctx,row){
        return Row(children:[
          SizedBox(width:30,child:Text('${row+1}')),
          Expanded(child:Row(children:List.generate(7,(col){
            String code=pattern[row][col];
            return Expanded(child:GestureDetector(onTap:(){pickPatternCell(row,col);},child:Container(margin:const EdgeInsets.all(2),height:40,decoration:BoxDecoration(border:Border.all(color:Colors.grey)),child:Center(child:Text(code))))));
          }))),
        ]);
      })),
      FilledButton.icon(onPressed:(){showAutoApplyDialog();},icon:const Icon(Icons.play_arrow),label:const Text('按指定日期自動排班'),style: FilledButton.styleFrom(minimumSize:const Size(double.infinity,50))),
    ]);
  }
  void pickPatternCell(int r,int c){
    showModalBottomSheet(context:context,builder:(ctx){
      return Column(mainAxisSize:MainAxisSize.min,children:defs.keys.map((k){return ListTile(title:Text(k),onTap:(){setState((){pattern[r][c]=k;}); save(); Navigator.pop(ctx);});}).toList());
    });
  }
  void showAutoApplyDialog(){
    ...
    showDialog(context:context,builder:(ctx){
      return AlertDialog(title:const Text('自動排更'),content:Text('由 ${DateFormat('yyyy-MM-dd').format(start)} 到 ${DateFormat('yyyy-MM-dd').format(end)}'),actions:[
  ...
  Widget reportTab(){
    ...
    Map<String,double> hrsPerCode={}; Map<String,double> allowPerCode={}; Map<String,double> otPerCode={}; Map<String,double> extraOTPerCode={};
    double totalHrs=0,totalAllow=0,totalOT=0,totalExtraOT=0; int workDays=0, offDays=0;
    for(int i=1;i<=dim;i++){
      DateTime d=DateTime(focused.year,focused.month,i);
      String k=DateFormat('yyyy-MM-dd').format(d);
      String? code=roster[k]; if(code==null) continue;
      var def=defs[code];
      hrsPerCode[code]=(hrsPerCode[code]??0)+(def?.hours??0);
      allowPerCode[code]=(allowPerCode[code]??0)+(def?.allowance??0);
      otPerCode[code]=(otPerCode[code]??0)+(def?.ot??0);
      extraOTPerCode[code]=(extraOTPerCode[code]??0)+(rosterOT[k]??0);
      totalHrs+=(def?.hours??0); totalAllow+=(def?.allowance??0); totalOT+=(def?.ot??0); totalExtraOT+=(rosterOT[k]??0);
      if((def?.hours??0)>0) workDays++; else offDays++;
    }
    ...
    return ListView(padding:const EdgeInsets.all(12),children:[
      ...
      Card(color:const Color(0xFFFFF3E0),child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        cnt.isEmpty?const Text('暫無'):Column(children:cnt.entries.map((e){var def=defs[e.key]; return Row(children:[Container(padding:const EdgeInsets.symmetric(horizontal:8,vertical:4),decoration:BoxDecoration(color:def?.color??Colors.orange,borderRadius:BorderRadius.circular(8)),child:Text(e.key,style:const TextStyle(color:Colors.white,fontWeight:FontWeight.bold))),const SizedBox(width:8),Expanded(child:Text('${def?.label??''} x ${e.value}次')),Text('${hrsPerCode[e.key]?.toStringAsFixed(1)??'0'}h')]);}).toList()),
        const Divider(), Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[Text('總日數 $dim天'),Text('返工 $workDays天'),Text('休 $offDays天')]),
      ]))),
      ...
      Card(color:const Color(0xFFE8F5E9),child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
       ...
       ...allowPerCode.entries.where((e)=>e.value>0).map((e)=>Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[Text('${e.key} 津貼'),Text('\$${e.value.toStringAsFixed(0)}')])),
      ]))),
  ...
  Widget settingsTab(){
    return ListView(padding:const EdgeInsets.all(16),children:[
      ...
      ...defs.entries.map((e){var d=e.value; return ListTile(leading:CircleAvatar(backgroundColor:d.color,child:Text(d.code,style:const TextStyle(color:Colors.white,fontSize:12))),title:Text('${d.code} - ${d.label}'),subtitle:Text('${d.start}-${d.end} ${d.hours}h 津貼\$${d.allowance} OT${d.ot}h'),trailing:Row(mainAxisSize:MainAxisSize.min,children:[IconButton(icon:const Icon(Icons.edit),onPressed:(){editShift(d);}),IconButton(icon:const Icon(Icons.delete),onPressed:(){setState((){defs.remove(e.key);}); save();})]));}),
      ...
      Card(child:Column(children:defs.entries.map((e){var d=e.value; return ListTile(title:Text('${d.code} 津貼 \$${d.allowance}'),subtitle:Slider(value:d.allowance.clamp(0,200),min:0,max:200,divisions:20,label:'\$${d.allowance.round()}',onChanged:(v){setState((){d.allowance=v;}); save();}),trailing:IconButton(icon:const Icon(Icons.delete),onPressed:(){setState((){d.allowance=0;}); save();}));}).toList())),
      const SizedBox(height:16),
  ...
  void editShift(ShiftDef? oldDef){
    ...
    showDialog(context:context,builder:(ctx){
      return StatefulBuilder(builder:(ctx2,setD){
        return AlertDialog(title:Text(oldDef==null?'新增':'編輯'),content:SingleChildScrollView(child:Column(children:[
          TextField(controller:codeCtrl,decoration:const InputDecoration(labelText:'代碼 無限制')),
          TextField(controller:labelCtrl,decoration:const InputDecoration(labelText:'內容')),
          Row(children:[Expanded(child:TextField(controller:hoursCtrl,decoration:const InputDecoration(labelText:'工時'),keyboardType:TextInputType.number)),const SizedBox(width:8),Expanded(child:TextField(controller:allowCtrl,decoration:const InputDecoration(labelText:'津貼'),keyboardType:TextInputType.number))]),
          Row(children:[Expanded(child:TextField(controller:otCtrl,decoration:const InputDecoration(labelText:'OT'),keyboardType:TextInputType.number)),const SizedBox(width:8),Expanded(child:TextField(controller:startCtrl,decoration:const InputDecoration(labelText:'開始')))]),
          TextField(controller:endCtrl,decoration:const InputDecoration(labelText:'結束')),
