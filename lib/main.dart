...
class _S extends State<MainPage>{
  ...
  int parseMins(String t){ try{ if(t=='') return -1; var p=t.split(':'); return int.parse(p[0])*60+int.parse(p[1]); }catch(_){return -1;} }
  double allowanceForShift(Shift s){ if(!s.work || s.start=='') return 0; int sm=parseMins(s.start); if(sm<0) return 0; Allowance? best; int bestDiff=10000; for(var a in allowances){ if(!a.enabled) continue; int diff=sm-a.mins; if(diff<0) diff+=1440; if(diff<bestDiff && diff<720){ bestDiff=diff; best=a; } } return best?.amount??0; }
  ...
  void editAllowanceDialog(Allowance? old){ TextEditingController nameC=TextEditingController(text: old?.name??''); TextEditingController startC=TextEditingController(text: old?.start??'07:00'); TextEditingController amountC=TextEditingController(text: (old?.amount??0).toString()); bool enabled=old?.enabled??true; showDialog(context: context, builder: (ctx)=>StatefulBuilder(builder: (ctx,setM){ return AlertDialog(title:Text(old==null?'新增津貼':'編輯津貼'), content:Column(mainAxisSize:MainAxisSize.min, children:[ TextField(controller: nameC, decoration:InputDecoration(labelText:'名稱 例如 早班津貼')), TextField(controller: startC, decoration:InputDecoration(labelText:'開始時間 HH:MM')), TextField(controller: amountC, keyboardType:TextInputType.number, decoration:InputDecoration(labelText:'金額 \$')), CheckboxListTile(title:Text('啟用'), value:enabled, onChanged:(v){ setM(()=>enabled=v!); }) ]), actions:[ TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('取消')), FilledButton(onPressed:(){ if(nameC.text=='') return; setState((){ var na=Allowance(nameC.text, startC.text, double.tryParse(amountC.text)??0, enabled); if(old==null) allowances.add(na); else { int idx=allowances.indexOf(old); allowances[idx]=na; } }); save(); Navigator.pop(ctx); },child:Text('保存'))]); })); }
  void showAllShifts(){ showDialog(context: context, builder: (ctx)=>AlertDialog(title:Text('全部班次'), content:SizedBox(width:400,height:500, child: ListView(children:[ for(var s in shifts) ListTile(leading:CircleAvatar(backgroundColor:s.color, child:Text(s.code)), title:Text('${s.code} ${s.name} ${s.start}-${s.end}'), subtitle:Text('津貼 \$${allowanceForShift(s)}'), trailing:Row(mainAxisSize:MainAxisSize.min, children:[ IconButton(icon:Icon(Icons.edit), onPressed:(){ Navigator.pop(ctx); editShiftDialog(s); }), IconButton(icon:Icon(Icons.delete), onPressed:(){ setState(()=>shifts.remove(s)); save(); Navigator.pop(ctx); showAllShifts(); }) ]))) ])), actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('關閉'))])); }
  void showAllAllowances(){ showDialog(context: context, builder: (ctx)=>AlertDialog(title:Text('全部津貼'), content:SizedBox(width:400,height:500, child: ListView(children:[ for(var a in allowances) ListTile(title:Text('${a.name} ${a.start}'), subtitle:Text('\$${a.amount} ${a.enabled?'啟用':''}'), trailing:Row(mainAxisSize:MainAxisSize.min, children:[ IconButton(icon:Icon(Icons.edit), onPressed:(){ Navigator.pop(ctx); editAllowanceDialog(a); }), IconButton(icon:Icon(Icons.delete), onPressed:(){ setState(()=>allowances.remove(a)); save(); Navigator.pop(ctx); showAllAllowances(); }) ]))) ])), actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text('關閉'))])); }
  ...
  void applyPattern(){ if(patterns.isEmpty) return; RosterPattern sel=patterns.first; DateTime sDate=DateTime(focused.year,focused.month,1); DateTime eDate=DateTime(focused.year,focused.month+1,0); showDialog(context: context, builder: (ctx)=>StatefulBuilder(builder: (ctx,setM){ return AlertDialog(title:Text('套用模式'), content:Column(mainAxisSize:MainAxisSize.min, children:[ DropdownButton<RosterPattern>(value:sel, items:[for(var p in patterns) DropdownMenuItem(value:p, child:Text(p.name))], onChanged:(v){ if(v!=null) setM(()=>sel=v); }) ]), actions:[ FilledButton(onPressed:(){ setState((){ int idx=0; for(DateTime d=sDate;!d.isAfter(eDate); d=d.add(Duration(days:1))){ roster[k(d)]=sel.codes[idx%sel.codes.length]; idx++; } save(); }); Navigator.pop(ctx); },child:Text('開始')) ]); })); }
  ...
  Widget buildCal(){
    ...
    return Scaffold(appBar: AppBar(leading: IconButton(icon:Icon(Icons.today), onPressed:goToday), title: InkWell(onTap:pickYM, child: Text('${focused.year}年${focused.month}月',style:TextStyle(fontWeight:FontWeight.w900,fontSize:20)))), body: Column(children:[
      ...
      Expanded(flex:3, child: GridView.builder(padding:EdgeInsets.all(4), physics:NeverScrollableScrollPhysics(), gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:7, childAspectRatio:0.9, crossAxisSpacing:4, mainAxisSpacing:4), itemCount:42, itemBuilder:(c,i){
        ...
        return GestureDetector(onTap:()=>setState(()=>selected=day), onLongPress:()=>editCell(day), child:Container(decoration:BoxDecoration(color:sh!=null?sh.color:Color(0xFFEEEEEE), borderRadius:BorderRadius.circular(8), border:isToday?Border.all(color:Colors.amber,width:3):isSel?Border.all(color:Colors.indigo,width:2):null), child:Stack(children:[
          if(isMon) Positioned(top:2,left:2,child:Container(color:Colors.black54,padding:EdgeInsets.symmetric(horizontal:2),child:Text('W${isoWeek(day)}',style:TextStyle(color:Colors.white,fontSize:7)))),
          Center(child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[ Text(day.day.toString(),style:TextStyle(fontWeight:FontWeight.bold,color:sh!=null?Colors.white:Colors.black)), if(sh!=null) Text(sh.code,style:TextStyle(color:Colors.white,fontSize:fs-1)), if(sh!=null && dayA>0) Text('\$${dayA.toStringAsFixed(0)}',style:TextStyle(color:Colors.white,fontSize:8)) ]))
        ])));
      })),
      Divider(height:1),
      Expanded(flex:2, child: Container(color:Colors.white, padding:EdgeInsets.all(12), child:ListView(children:[
        Row(children:[ Container(padding:EdgeInsets.symmetric(horizontal:12,vertical:6),decoration:BoxDecoration(color:sel?.color??Colors.grey.shade300,borderRadius:BorderRadius.circular(20)),child:Text('${DateFormat('MM/dd EEEE').format(selected)} ${roster[k(selected)]??'未排'}')), Spacer(), FilledButton(onPressed:()=>editCell(selected),child:Text('編輯')) ]),
        if(sel!=null) Padding(padding:EdgeInsets.only(top:8), child: Text('早更 ${sel.start}-${sel.end} ${sel.h}h 津貼 \$${allowanceForShift(sel).toStringAsFixed(0)} + 交通 \$${sel.work?transB:0} = \$${(allowanceForShift(sel)+(sel.work?transB:0)).toStringAsFixed(0)}')),
      ]))),
    ]));
  }
  ...
  Widget buildReport(){
    ...
    return Scaffold(appBar:AppBar(title:Text('${mon.year}年${mon.month}月 報表')), body:ListView(padding:EdgeInsets.all(16), children:[
      ...
      Card(color:Colors.indigo.shade50, child:ListTile(title:Text('總額'), trailing:Text('\$${(totA+totTrans).toStringAsFixed(0)}',style:TextStyle(fontSize:22,fontWeight:FontWeight.bold)))),
    ]));
  }

  Widget buildSetting(){
    var showShifts = shifts.take(5).toList();
    var showAllow = allowances.take(5).toList();
    return Scaffold(appBar:AppBar(title:Text('設定 V7.2 無彩虹')), body:ListView(padding:EdgeInsets.all(12), children:[
      Text('自定班次 (只顯示5行)',style:TextStyle(fontWeight:FontWeight.bold)),
      Card(child: Column(children:[
        for(var s in showShifts) ListTile(dense:true, leading:CircleAvatar(backgroundColor:s.color, radius:16, child:Text(s.code,style:TextStyle(color:Colors.white,fontSize:10))), title:Text('${s.code} ${s.name} ${s.start}-${s.end}',style:TextStyle(fontSize:12)), subtitle:Text('自動津貼 \$${allowanceForShift(s).toStringAsFixed(0)}'), trailing:Row(mainAxisSize:MainAxisSize.min, children:[ IconButton(icon:Icon(Icons.edit,size:16), onPressed:()=>editShiftDialog(s)), IconButton(icon:Icon(Icons.delete,size:16), onPressed:(){ setState(()=>shifts.remove(s)); save(); }) ])),
        if(shifts.length>5) ListTile(title:Center(child: Text('還有 ${shifts.length-5} 個... 查看全部',style:TextStyle(color:Colors.indigo))), onTap:showAllShifts),
      ])),
      ...
      Card(child: Column(children:[
        for(var a in showAllow) ListTile(dense:true, leading:CircleAvatar(radius:16, child:Text(a.start,style:TextStyle(fontSize:8))), title:Text('${a.name} 開始${a.start}',style:TextStyle(fontSize:12)), subtitle:Text('\$${a.amount}'), trailing:Row(mainAxisSize:MainAxisSize.min, children:[ IconButton(icon:Icon(Icons.edit,size:16), onPressed:()=>editAllowanceDialog(a)), IconButton(icon:Icon(Icons.delete,size:16), onPressed:(){ setState(()=>allowances.remove(a)); save(); }) ])),
        if(allowances.length>5) ListTile(title:Center(child: Text('還有 ${allowances.length-5} 個... 查看全部')), onTap:showAllAllowances),
      ])),
      Row(children:[ Expanded(child: FilledButton.icon(onPressed:()=>editAllowanceDialog(null), icon:Icon(Icons.add), label:Text('新增津貼'))), SizedBox(width:8), Expanded(child: OutlinedButton(onPressed:(){ setState(()=>allowances=[ Allowance('早班津貼','06:00',20,true), Allowance('中班津貼','14:00',0,true), Allowance('夜班津貼','22:00',80,true), Allowance('通宵津貼','23:30',120,true), ]); save(); }, child:Text('重置預設')))]),

      Divider(),
      Card(child: ListTile(title:Text('OT時薪 \$${otR}'), trailing:SizedBox(width:90, child:TextField(decoration:InputDecoration(border:OutlineInputBorder(), isDense:true), keyboardType:TextInputType.number, onSubmitted:(v){ double? vv=double.tryParse(v); if(vv!=null){ setState(()=>otR=vv); save(); } })))),
      Card(child: ListTile(title:Text('交通 \$${transB}'), trailing:SizedBox(width:90, child:TextField(decoration:InputDecoration(border:OutlineInputBorder(), isDense:true), keyboardType:TextInputType.number, onSubmitted:(v){ double? vv=double.tryParse(v); if(vv!=null){ setState(()=>transB=vv); save(); } })))),
      for(var p in patterns) Card(child: ListTile(title:Text(p.name), trailing:IconButton(icon:Icon(Icons.delete), onPressed:(){ setState(()=>patterns.remove(p)); save(); }))),
      FilledButton.icon(onPressed:createPattern, icon:Icon(Icons.add), label:Text('新增模式')),
