...
class _S extends State<MainPage>{
  ...
  String? currentGoogleEmail, selectedCalendarId, selectedCalendarName;
  List<calendar.CalendarListEntry> calList=[];
  final GoogleSignIn _gs = GoogleSignIn(scopes: [drive.DriveApi.driveAppdataScope, drive.DriveApi.driveFileScope, calendar.CalendarApi.calendarScope]);
  ...
  Future<void> pickGoogleAccountAndCalendars() async{
    try{
      var acc=await _gs.signIn(); if(acc==null) return;
      setState(()=>currentGoogleEmail=acc.email);
      var auth=await _gs.authenticatedClient();
      var calApi=calendar.CalendarApi(auth!);
      var cals=await calApi.calendarList.list();
      setState(()=>calList=cals.items??[]);
      if(calList.isNotEmpty && selectedCalendarId==null){
  ...
  Widget buildReport(){
    ...
    double cum=prevBalance; List<Map<String,dynamic>> rows=[]; var sorted=weekHours.keys.toList()..sort(); for(var wk in sorted){ double actual=weekHours[wk]!; double diff=actual-weeklyStandard; cum+=diff; rows.add({'wk':wk,'actual':actual,'diff':diff,'cum':cum,'days':weekDays[wk]}); }
    return SafeArea(child:ListView(padding:EdgeInsets.all(12), children:[
      ...
      Card(child:ListTile(title:Text('承上 $prevBalance h + 本月 ${totHours.toStringAsFixed(1)}h = 餘額 ${cum.toStringAsFixed(1)}h'))),
      Card(color:Color(0xFFE3F2FD), child:Column(children:[ ListTile(title:Text('每週餘額',style:TextStyle(fontWeight:FontWeight.bold))), for(var r in rows) ListTile(dense:true, leading:CircleAvatar(radius:18, child:Text('W${r['wk']}',style:TextStyle(fontSize:10))), title:Text('${r['actual'].toStringAsFixed(1)}h - ${weeklyStandard}h = ${r['diff'].toStringAsFixed(1)}h'), trailing:Text('餘 ${r['cum'].toStringAsFixed(1)}h', style:TextStyle(fontWeight:FontWeight.bold))), if(rows.isEmpty) Padding(padding:EdgeInsets.all(12), child:Text('本月無資料')), ])),
    ]));
  }
  ...
  Widget buildSetting(){
    ...
    return SafeArea(child:ListView(padding:EdgeInsets.all(12), children:[
      ...
      Card(color:Color(0xFFE8F5E9), child:Column(children:[
        ...
        Divider(),
        ListTile(title:Text('Google帳號'), subtitle:Text(currentGoogleEmail==null?'未登入':currentGoogleEmail!), trailing:IconButton(icon:Icon(Icons.account_circle), onPressed:pickGoogleAccountAndCalendars)),
        if(currentGoogleEmail!=null) ListTile(title:Text('已選日曆'), subtitle:Text(selectedCalendarName??'未選'), trailing:TextButton(onPressed:pickGoogleAccountAndCalendars, child:Text('更換'))),
        SwitchListTile(title:Text('Google日曆同步'), subtitle:Text(googleCalEnabled?'已啟用 - 輸入即自動同步':'未啟用'), value:googleCalEnabled, onChanged:(v){ setState(()=>googleCalEnabled=v); save(); if(v) pickGoogleAccountAndCalendars(); }),
      ...
      Card(color:Color(0xFFFFF8E1), child:Column(children:[
        ...
        ListTile(title:Text('交通津貼'), trailing:SizedBox(width:100, child:TextField(controller:TextEditingController(text:transB.toString()), decoration:InputDecoration(prefixText:'\$', border:OutlineInputBorder(), isDense:true), onSubmitted:(v){ var vv=double.tryParse(v); if(vv!=null){ setState(()=>transB=vv); save(); } }))),
        Divider(),
        for(var a in allowances) ListTile(dense:true, leading:Icon(a.enabled?Icons.check_circle:Icons.cancel, color:a.enabled?Colors.green:Colors.grey), title:Text('${a.name} ${a.start} \$${a.amount.toStringAsFixed(0)}'), trailing:Row(mainAxisSize:MainAxisSize.min, children:[ IconButton(icon:Icon(Icons.edit,size:20), onPressed:()=>editAllowDialog(a)), IconButton(icon:Icon(Icons.delete,size:20), onPressed:(){ setState(()=>allowances.remove(a)); save(); }) ])),
      ])),
      Divider(),
      ...
      SizedBox(height:8),
      FilledButton.icon(style: FilledButton.styleFrom(backgroundColor:Color(0xFF6750A4), minimumSize:Size(double.infinity,50)), onPressed:applyPattern, icon:Icon(Icons.play_arrow), label:Text('按指定日期自動排班 (套用排更)')),
      SizedBox(height:80),
    ]));
  }
}
