...
Map<String,String> getHolidays(int year,String region){ Map<String,String> m={}; if(region=='無') return m; if(region=='香港'){ m['$year-01-01']='元旦'; m['$year-05-01']='勞動節'; m['$year-07-01']='回歸'; m['$year-10-01']='國慶'; m['$year-12-25']='聖誕'; if(year==2026){ m.addAll({'2026-02-17':'初一','2026-02-18':'初二','2026-02-19':'初三','2026-04-05':'清明','2026-05-24':'佛誕','2026-06-19':'端午'}); } } return m; }
...
Color bg;
if(isToday) bg=todayBgColor; else if(!inM) bg=const Color(0xFFF5F5F0); else if(def!=null) bg=def.color.withOpacity(0.18); else bg=const Color(0xFFFFF0D0);
...
Text('${day.day}',style:TextStyle(fontWeight:isToday?FontWeight.w900:FontWeight.bold,fontSize:calendarFontSize-1)),
...
const Divider(), ListTile(title:const Text('總津貼 (含自定)'),trailing:Text('\$${(allow+fixedExtra).toStringAsFixed(0)}',style:const TextStyle(fontWeight:FontWeight.bold,fontSize:16))), FilledButton(onPressed:exportReport,child:const Text('匯出報表 CSV')),
]);
}
Widget settingsTab(){
...
const Divider(), const ListTile(title:Text('桌面Widget已接通 home_widget'),subtitle:Text('今日/明日自動更新到桌面 RosterWidgetProvider')),
]);
}
