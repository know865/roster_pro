...
class _S extends State<MainPage>{
  ...
  void editAllowDialog(Allowance? old){
    ...
    showDialog(context:context, builder:(ctx)=>StatefulBuilder(builder:(ctx,setM){
      return AlertDialog(title:Text(old==null?'新增津貼 - 自定開始時間':'編輯津貼 - 自定開始時間'), content:Column(mainAxisSize:MainAxisSize.min, children:[
        TextField(controller:nC, decoration:InputDecoration(labelText:'名稱')),
        TextField(controller:sC, decoration:InputDecoration(labelText:'開始時間 HH:MM 例如 22:00')),
  ...
  Widget buildCal(){
    ...
    return Column(children:[
      SafeArea(child:Padding(padding:EdgeInsets.all(12), child:Row(children:[
        ...
        Spacer(),
        IconButton(icon:Icon(Icons.grid_view), onPressed:createPattern),
        IconButton(icon:Icon(Icons.playlist_add_check), onPressed:applyPattern),
        IconButton(icon:Icon(Icons.share), onPressed:(){}),
      ]))),
      Row(children:[ for(var w in ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']) Expanded(child:Center(child:Text(w,style:TextStyle(fontWeight:FontWeight.bold,fontSize:fs)))) ]),
      Column(children:[
        for(int r=0;r<6;r++) Row(children:[
          for(int c=0;c<7;c++) Builder(builder:(_){
            ...
            double dayA=sh!=null?allowFor(sh)+(sh.work?transB:0)+(extra[key]!=null?extra[key]!*otR:0):0;
            ...
            return Expanded(child: GestureDetector(onTap:(){ setState(()=>selected=day); }, child: Container(height:64, margin:EdgeInsets.all(4), decoration:BoxDecoration(color:bg, borderRadius:BorderRadius.circular(12), border:isSel?Border.all(color:Color(0xFFFFC107),width:3):null), child:Stack(children:[
              ...
              Center(child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[
                Text(day.day.toString(),style:TextStyle(fontWeight:FontWeight.bold,fontSize:fs)),
                if(sh!=null) Text(sh.code,style:TextStyle(fontSize:fs-2,color:sh.color)),
                if(dayA>0&&sh!=null) Text('\$${dayA.toStringAsFixed(0)}',style:TextStyle(fontSize:7)),
              ])),
            ]))));
          })
        ])
      ...
      Padding(padding:EdgeInsets.symmetric(horizontal:12,vertical:6), child:Row(children:[
        Expanded(child: Container(padding:EdgeInsets.symmetric(horizontal:16,vertical:10), decoration:BoxDecoration(color:Color(0xFFE0E0E0), borderRadius:BorderRadius.circular(24)), child:Text('${DateFormat('MM/dd EEEE', 'en_US').format(selected)} ${roster[k(selected)]??'未排'} ${roster[k(selected)]!=null?'津貼\$${allowFor(getS(roster[k(selected)]!)).toStringAsFixed(0)}':''}',style:TextStyle(fontWeight:FontWeight.bold))))),
        SizedBox(width:8),
  ...
  Widget buildReport(){
    ...
    return ListView(padding:EdgeInsets.all(16), children:[
      ...
      Card(child:ListTile(title:Text('返工 $cW日'), subtitle:Text('${totH}h'))),
  ...
  Widget buildSetting(){
    return ListView(padding:EdgeInsets.all(12), children:[
      ...
      for(var s in shifts) Card(child:ListTile(
        ...
        subtitle:Text('\$${allowFor(s).toStringAsFixed(1)}\n${s.work?'有交通':'無交通'}'),
      ...
      Text('系統會按班次開始時間，自動匹配最接近嘅津貼',style:TextStyle(fontSize:11,color:Colors.grey)),
