...
String get detailTime => isAllDay ? '全天 {start}-{hours.toStringAsFixed(1)}h';
}
...
if (d == 1) return '{lunarMonths[m - 1]}月';
...
if (region == '香港') {
m['${year}-01-01'] = '元旦'; m['${year}-05-01'] = '勞動節';
m['${year}-07-01'] = '回歸'; m['${year}-10-01'] = '國慶';
m['${year}-12-25'] = '聖誕'; if (year == 2024) { m.addAll({'2024-02-10': '初一', '2024-02-11': '初二', '2024-02-12': '初三', '2024-04-04': '清明', '2024-05-15': '佛誕', '2024-06-10': '端午', '2024-09-18': '中秋翌日', '2024-10-11': '重陽', '2024-12-26': '聖誕後'}); } else if (year == 2025) { m.addAll({'2025-01-29': '初一', '2025-01-30': '初二', '2025-01-31': '初三', '2025-04-04': '清明', '2025-05-05': '佛誕', '2025-05-31': '端午', '2025-10-07': '中秋翌日', '2025-10-29': '重陽'}); } else if (year == 2026) { m.addAll({'2026-02-17': '初一', '2026-02-18': '初二', '2026-02-19': '初三', '2026-04-05': '清明', '2026-05-24': '佛誕', '2026-06-19': '端午', '2026-09-26': '中秋翌日', '2026-10-18': '重陽'}); } else if (year == 2027) { m.addAll({'2027-02-06': '初一', '2027-02-07': '初二', '2027-02-08': '初三', '2027-04-05': '清明', '2027-05-13': '佛誕', '2027-06-09': '端午', '2027-09-16': '中秋翌日', '2027-10-08': '重陽'}); } else if (year == 2028) { m.addAll({'2028-01-26': '初一', '2028-01-27': '初二', '2028-01-28': '初三', '2028-04-04': '清明', '2028-05-01': '佛誕', '2028-05-27': '端午', '2028-10-04': '中秋翌日', '2028-10-26': '重陽'}); } else if (year == 2029) { m.addAll({'2029-02-13': '初一', '2029-02-14': '初二', '2029-02-15': '初三', '2029-04-05': '清明', '2029-05-20': '佛誕', '2029-06-16': '端午', '2029-09-23': '中秋翌日', '2029-10-15': '重陽'}); } else if (year == 2030) { m.addAll({'2030-02-03': '初一', '2030-02-04': '初二', '2030-02-05': '初三', '2030-04-05': '清明', '2030-05-09': '佛誕', '2030-06-05': '端午', '2030-09-12': '中秋翌日', '2030-10-04': '重陽'}); } else if (year &gt;= 2031) { m.addAll({'${year}-02-10': '春節', '${year}-04-05': '清明', '${year}-05-15': '佛誕', '${year}-06-10': '端午', '${year}-09-18': '中秋', '${year}-10-11': '重陽'}); } } if (region == '中國內地') { m['${year}-01-01'] = '元旦'; m['${year}-05-01'] = '勞動節'; m['${year}-10-01'] = '國慶'; }
if (region == '台灣') { m['${year}-01-01'] = '元旦'; m['${year}-02-28'] = '和平紀念'; m['${year}-10-10'] = '國慶'; } if (region == '美國') { m['${year}-01-01'] = 'New Year'; m['${year}-07-04'] = 'Independence'; m['${year}-11-11'] = 'Veterans'; m['`${year}-12-25'] = 'Christmas'; }
...
try {
await realChannel.invokeMethod('requestManageStorage');
} catch () {}
await updateWidget();
});
}
...
var mh = sp.getString('manualHolidays'); if (mh != null) { try { manualHolidays = Map<String, String>.from(jsonDecode(mh)); } catch () {} }
setState(() {
...
Expanded(child: Text('latex
{y}年', textAlign: TextAlign.center, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))), IconButton(icon: const Icon(Icons.add), onPressed: () =&gt; setD(() =&gt; y++)) ]), Wrap(spacing: 8, children: List.generate(12, (i) { int mon = i + 1; return ChoiceChip(label: Text('

{mon}月'), selected: mon == m, onSelected: (_) => setD(() => m = mon));
}))
 ]),
actions: [
...
var devReq = await calendarPlugin.requestPermissions();
...
return AlertDialog(
title: Text('選擇寫入日曆 ({googleCals.length})', style: const TextStyle(fontWeight: FontWeight.bold)),
...
// 智能掃描所有 [RosterPro] 事件並刪除（動態計算範圍）
DateTime scanStart, scanEnd;
if (roster.isEmpty) {
int currentYear = DateTime.now().year;
scanStart = DateTime(currentYear - 10, 1, 1);
scanEnd = DateTime(currentYear + 10, 12, 31);
} else {
DateTime minDate = DateTime(2100);
DateTime maxDate = DateTime(2000);
for (String dateKey in roster.keys) {
DateTime d = DateTime.parse(dateKey);
if (d.isBefore(minDate)) minDate = d;
if (d.isAfter(maxDate)) maxDate = d;
}
scanStart = DateTime(minDate.year - 2, 1, 1);
scanEnd = DateTime(maxDate.year + 2, 12, 31);
}
...
try {
await _calendarPlugin.deleteEvent(rosterCalendarId!, id);
del++;
} catch () {}
}
...
{def.start}-{note.isNotEmpty ? '\n記事: latex
note' : ''}'; title = '

{def.code} {def.end}latex
{note.isNotEmpty ? ' | 
...
try { await _calendarPlugin.deleteEvent(rosterCalendarId!, e.eventId!); del++; } catch () {}
}
}
...
String tag = '[RosterPro]${dateKey}'; bool allDayFlag = def.isAllDay || def.code == 'O'; String desc, title; if (allDayFlag) { desc = '$tag\n$customName\n班次: ${def.code} ${def.label}\n類型: 全天${note.isNotEmpty ? '\n記事: $note' : ''}'; title = '${def.code}${note.isNotEmpty ? ' | $note' : ''}';
} else {
...
try { await _calendarPlugin.deleteEvent(_rosterCalendarId!, googleEventIdMap[k]); } catch () {}
...
{v.allowance}' : ''}', style: const TextStyle(color: Colors.black87, fontSize: 28, fontWeight: FontWeight.w600));
...
String path = 'latex
{dcimDir.path}/Roster_

{focused.year}latex
{focused.month.toString().padLeft(2, '0')}_

{DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.jpg';
File f = File(path);
await f.writeAsBytes(jpgBytes);
try { await realChannel.invokeMethod('scanImage', {'path': path}); } catch () {}

if (mounted) {
ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('截圖已保存到相冊')));
await Share.shareXFiles([XFile(path)], text: '{focused.month}月 latex
customName'); } } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('截圖失敗 
...
if (dir == null) return;
String fileName = 'roster_pro_full_dir/dir/_lastBackupPath')));
}
...
sb.writeln('總工時, $hrs'); sb.writeln('OT, $ot');
sb.writeln('津貼, ${allow + ot * overtimeRate}'); sb.writeln(''); sb.writeln('每週工時統計:'); sb.writeln('週次, 工時, 標準, 差額'); for (var e in weeklyHours.entries) { double diff = e.value - standardWeeklyHours; sb.writeln('W${e.key}, ${e.value.toStringAsFixed(1)}h, ${standardWeeklyHours}h, ${diff &gt;= 0 ? '+' : ''}${diff.toStringAsFixed(1)}h');
}
} else {
...
String fileName = 'report_latex
{isYearReport ? 'year

{focused.year}' : '{focused.month.toString().padLeft(2, '0')}'}.csv';
String path = 'fileName';
final bytes = <int>[0xEF, 0xBB, 0xBF, ...utf8.encode(sb.toString())];
await File(path).writeAsBytes(bytes);
if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('已匯出 latex
path'))); } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('匯出失敗 
...
return AlertDialog(
title: Text(yearMode ? 'queryYear年queryYear年' : 'queryMonth月', textAlign: TextAlign.center, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
...
subtitle: Text('{en.value.data.length * 7}天)'),
...
content: Text(
'模式: latex
{savedPatterns[selectedIdx].name}\n' '模式行數: 
...
Flexible(child: FittedBox(fit: BoxFit.scaleDown, child: Text('{focused.month}月', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)))),
...
child: Text('${day.day}', style: TextStyle(fontWeight: isToday ? FontWeight.w900 : FontWeight.bold, fontSize: calendarFontSize, color: inM ? Colors.black : Colors.grey)), ), if (code != null) FittedBox( fit: BoxFit.scaleDown, child: Container( padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: def?.color ?? Colors.orange, borderRadius: BorderRadius.circular(4)), child: Text(code, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.bold)), ), ) else const SizedBox(height: 14), if (showLunar && lunarText.isNotEmpty && inM) FittedBox( fit: BoxFit.scaleDown, child: Text(lunarText, style: TextStyle(fontSize: 9, color: Colors.grey[700])), ) else const SizedBox(height: 10), SizedBox( height: 6, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [ if (isHol) Container(width: 5, height: 5, margin: const EdgeInsets.symmetric(horizontal: 0.5), decoration: BoxDecoration(color: holidayDotColor, shape: BoxShape.circle)), if (hasNote) Container(width: 5, height: 5, margin: const EdgeInsets.symmetric(horizontal: 0.5), decoration: const BoxDecoration(color: Colors.blue, shape: BoxShape.circle)), ]), ), ] ) ) ); } ) ) ]); } ) ) ]) ) ) ), Container( width: double.infinity, padding: const EdgeInsets.fromLTRB(12, 12, 12, 16), decoration: const BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: Color(0xFFE0E0E0)))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ Row(children: [ Expanded(child: Text('${roster[selKey] ?? '未排班'}${isHoliday(selectedDay) ? ' [${holidayName(selectedDay)}]' : ''} ${extraType.isNotEmpty ? '[$extraType]' : ''}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis)),
...
Text('1. 班次：${selDef != null ? '(${selDef.code}) ${selDef.label}' : ''} ${isHoliday(selectedDay) ? '[${holidayName(selectedDay)}]' : ''}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text('2. 時間：${selDef != null ? (selDef.isAllDay ? '全天' : '${selDef.start}-${selDef.end}') : ''} | 工時：${selDef?.hours ?? 0}h', style: const TextStyle(fontSize: 12)), const SizedBox(height: 4), Text('3. 班次津貼：${selDef != null && selDef.hasAllowance ? '有 `${selDef.allowance}' : '無'}', style: const TextStyle(fontSize: 12)),
const SizedBox(height: 4),
Text('4. 額外津貼名稱：latex
{extraType.isNotEmpty ? extraType : '無'} 金額：\

{(rosterExtra[selKey] ?? 0).toStringAsFixed(1)}', style: const TextStyle(fontSize: 12, color: Colors.deepPurple, fontWeight: FontWeight.w600)),
const SizedBox(height: 4),
Text('5. OT：{(rosterExtraHrs[selKey] ?? 0).toStringAsFixed(1)}h', style: const TextStyle(fontSize: 12)),
const SizedBox(height: 4),
...
return AlertDialog(
title: Text(oldDef == null ? '新增班次' : '編輯 latex
{oldDef.code}'), content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [ TextField(controller: codeCtrl, decoration: const InputDecoration(labelText: '代號')), TextField(controller: labelCtrl, decoration: const InputDecoration(labelText: '名稱')), const SizedBox(height: 10), Row(children: [ Expanded(child: InkWell(onTap: () async { if (isAllDay) return; TimeOfDay? t = await _pickWheelTime(ctx2, TimeOfDay(hour: int.parse(startCtrl.text.split(':')[0]), minute: int.parse(startCtrl.text.split(':')[1]))); if (t != null) { setS(() =&gt; startCtrl.text = '

{t.hour.toString().padLeft(2, '0')}:latex
{t.minute.toString().padLeft(2, '0')}'); calcHours(); } }, child: InputDecorator(decoration: const InputDecoration(labelText: '開始 HH:mm', border: OutlineInputBorder()), child: Text(startCtrl.text)))), const SizedBox(width: 8), Expanded(child: InkWell(onTap: () async { if (isAllDay) return; TimeOfDay? t = await _pickWheelTime(ctx2, TimeOfDay(hour: int.parse(endCtrl.text.split(':')[0]), minute: int.parse(endCtrl.text.split(':')[1]))); if (t != null) { setS(() =&gt; endCtrl.text = '

{t.hour.toString().padLeft(2, '0')}:latex
{t.minute.toString().padLeft(2, '0')}'); calcHours(); } }, child: InputDecorator(decoration: const InputDecoration(labelText: '結束 HH:mm', border: OutlineInputBorder()), child: Text(endCtrl.text)))), ]), const SizedBox(height: 8), Row(children: [ Checkbox(value: isAllDay, onChanged: (v) { setS(() { isAllDay = v ?? false; if (isAllDay) { startCtrl.text = '00:00'; endCtrl.text = '00:00'; } else { calcHours(); } }); }), const Text('全天', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(width: 8), const Expanded(child: Text('核實全天後時間變00:00，工時可任意輸入', style: TextStyle(fontSize: 10, color: Colors.grey))), ]), Row(children: [ Expanded(child: SizedBox(height: 78, child: TextField(controller: hoursCtrl, decoration: InputDecoration(labelText: '工時', border: const OutlineInputBorder(), helperText: isAllDay ? '全天可任意輸入' : ' ', helperStyle: const TextStyle(fontSize: 10)), keyboardType: TextInputType.number))), const SizedBox(width: 8), Expanded(child: SizedBox(height: 78, child: TextField(controller: otCtrl, decoration: const InputDecoration(labelText: 'OT', border: OutlineInputBorder(), helperText: ' ', helperStyle: TextStyle(fontSize: 10)), keyboardType: TextInputType.number))), ]), const SizedBox(height: 12), Row(children: [Checkbox(value: hasAllow, onChanged: (v) =&gt; setS(() =&gt; hasAllow = v ?? false)), const Text('有津貼核實', style: TextStyle(fontWeight: FontWeight.bold))]), if (hasAllow) TextField(controller: allowCtrl, decoration: const InputDecoration(labelText: '津貼金額', prefixText: '\
...
content: SizedBox(width: 300, child: ListView(shrinkWrap: true, children: [
ListTile(title: const Text('當前版面'), subtitle: Text('{en.value.data.length}行'), leading: const Icon(Icons.folder), onTap: () => Navigator.pop(ctx, en.key))),
 ])),
actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消'))]
);
});
...
const SizedBox(height: 8),
Row(mainAxisAlignment: MainAxisAlignment.center, children: [
FilledButton.tonalIcon(icon: const Icon(Icons.folder), label: Text('已存模式latex
{savedPatterns.isEmpty ? '' : '(

{savedPatterns.length})'}'), onPressed: () {
...
var ctrl = TextEditingController(text: '模式_latex
{DateFormat('MMdd_HHmm').format(DateTime.now())}'); showDialog(context: context, builder: (ctx) =&gt; AlertDialog( title: const Text('另存排更模式 自定義名稱'), content: TextField(controller: ctrl, decoration: const InputDecoration(labelText: '模式名稱', border: OutlineInputBorder())), actions: [ TextButton(onPressed: () =&gt; Navigator.pop(ctx), child: const Text('取消')), FilledButton(onPressed: () { String n = ctrl.text.trim(); if (n.isEmpty) return; if (savedPatterns.any((e) =&gt; e.name == n)) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('名稱 
...
child: Row(mainAxisSize: MainAxisSize.min, children: [
Text('正在編輯: selectedPatternCode (點下方格子填入)', style: const TextStyle(fontSize: 12, color: Colors.grey)),
),
...
Flexible(
child: Text(
'${year}年${isYearReport ? ' 全年' : ' ${month}月'}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis, ), ), const Icon(Icons.arrow_drop_down, size: 22), ]), ), ), ), const SizedBox(width: 4), SegmentedButton&lt;bool&gt;( segments: const [ButtonSegment(value: false, label: Text('本月')), ButtonSegment(value: true, label: Text('全年'))], selected: {isYearReport}, onSelectionChanged: (s) { setState(() =&gt; isYearReport = s.first); }, style: ButtonStyle( padding: WidgetStateProperty.all(const EdgeInsets.symmetric(horizontal: 8)), textStyle: WidgetStateProperty.all(const TextStyle(fontSize: 13)), visualDensity: VisualDensity.compact, ), ), ]), const SizedBox(height: 8), if (isYearReport) Card(color: const Color(0xFFE3F2FD), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ Text('全年總工時 ${totalYearHrs.toStringAsFixed(1)}h', style: const TextStyle(fontWeight: FontWeight.bold)),
const Divider(),
...
const Divider(),
Text('總工時 ${hrs.toStringAsFixed(1)}h / 承上 ${carry}h / 合計 ${(hrs + carry).toStringAsFixed(1)}h'), ]))), Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ Text('承上 $carry h + 本月 $hrs h = ${carry + hrs}h', style: const TextStyle(fontWeight: FontWeight.bold)),
const Divider(),
...
Text('W${e.key}', style: const TextStyle(fontWeight: FontWeight.bold)), const SizedBox(width: 8), Text('${e.value.toStringAsFixed(1)}h +承上${avgCarry.toStringAsFixed(1)} = ${adjusted.toStringAsFixed(1)}h'),
const Spacer(),
...
...extraByType.entries.map((e) => Row(children: [Text('類別: latex
{e.key}'), const Spacer(), Text('\

{e.value.toStringAsFixed(1)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.deepPurple))])),
Row(children: [Text('OT{overtimeRate.toStringAsFixed(0)}'), const Spacer(), Text('${otAmount.toStringAsFixed(1)}')]), const Divider(), ...extraAllowances.map((e) =&gt; Row(children: [Text(e.name), const Spacer(), Text('\${e.amount}'), IconButton(icon: const Icon(Icons.delete, size: 16), onPressed: () { setState(() => extraAllowances.removeAt(extraAllowances.indexOf(e))); save(); })])),
const Divider(),
...
onPressed: () {
var dCtrl = TextEditingController(text: '${focused.year}-'); var nCtrl = TextEditingController(text: '農曆節日'); showDialog(context: context, builder: (ctx) =&gt; AlertDialog( title: const Text('新增農曆/手動假期'), content: Column(mainAxisSize: MainAxisSize.min, children: [ SizedBox(height: 56, child: TextField(controller: dCtrl, decoration: const InputDecoration(labelText: '日期 (yyyy-MM-dd)', isDense: true, border: OutlineInputBorder()))), const SizedBox(height: 8), SizedBox(height: 56, child: TextField(controller: nCtrl, decoration: const InputDecoration(labelText: '假期名稱', isDense: true, border: OutlineInputBorder()))), ]), actions: [ TextButton(onPressed: () =&gt; Navigator.pop(ctx), child: const Text('取消')), FilledButton(onPressed: () { if (dCtrl.text.isNotEmpty && nCtrl.text.isNotEmpty) { setState(() =&gt; manualHolidays[dCtrl.text.trim()] = nCtrl.text.trim()); save(); Navigator.pop(ctx); } }, child: const Text('新增')), ] )); }, )), ]), if (manualHolidays.isNotEmpty) ...[ const Divider(), const Text('已添加的手動假期:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)), ...manualHolidays.entries.map((e) =&gt; ListTile( dense: true, title: Text('${e.key} - ${e.value}', style: const TextStyle(fontSize: 12)), trailing: IconButton(icon: const Icon(Icons.delete, size: 18), onPressed: () { setState(() =&gt; manualHolidays.remove(e.key)); save(); }), )), ], SwitchListTile( title: const Text('顯示農曆'), value: showLunar, onChanged: (v) { setState(() =&gt; showLunar = v); save(); }, ), ]))), const SizedBox(height: 16), const Text('日曆同步', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [ SwitchListTile(title: const Text('啟用日曆同步'), subtitle: Text(googleSyncEnabled ? '已授權' : '未授權'), value: googleSyncEnabled, onChanged: (v) async { if (v) { await _requestGooglePerm(); } else { setState(() =&gt; googleSyncEnabled = false); save(); } }), SwitchListTile(title: const Text('自動同步(全刪重建)'), value: autoSync, onChanged: googleSyncEnabled ? (v) { setState(() =&gt; autoSync = v); save(); } : null), Row(children: [ Expanded(child: OutlinedButton.icon(onPressed: googleSyncEnabled ? () =&gt; _syncToGoogle() : null, icon: const Icon(Icons.sync), label: const Text('手動同步'))), const SizedBox(width: 8), Expanded(child: OutlinedButton.icon(onPressed: () { setState(() =&gt; googleSyncEnabled = false); save(); }, icon: const Icon(Icons.link_off), label: const Text('取消'))) ]), Text('當前: $_rosterCalendarName\nID: ${_rosterCalendarId ?? '未選'}', style: const TextStyle(fontSize: 11, color: Colors.grey)), const SizedBox(height: 8), SizedBox(width: double.infinity, child: OutlinedButton.icon(icon: const Icon(Icons.list), label: const Text('選擇日曆'), onPressed: () async { await _pickGoogleCalendarDialog(); })), SizedBox(width: double.infinity, child: OutlinedButton.icon(icon: const Icon(Icons.security), label: const Text('重新請求日曆權限'), onPressed: () async { await handleCalendarPermission(silent: false); setState(() {}); })), const Divider(), Container( width: double.infinity, padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.red.withOpacity(0.08), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.red.withOpacity(0.3))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ const Text('⚠️ 全清重建（救援用）', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red, fontSize: 13)), const Text('• 刪除所有 [RosterPro] 事件\n• App 排班資料不受影響', style: TextStyle(fontSize: 10, color: Colors.black54)), const SizedBox(height: 8), SizedBox(width: double.infinity, child: FilledButton.icon( onPressed: googleSyncEnabled ? _forceFullResync : null, icon: const Icon(Icons.cleaning_services, size: 18), label: const Text('執行全清重建'), style: FilledButton.styleFrom(backgroundColor: Colors.red), )), ]) ), ]))), const SizedBox(height: 16), const Text('清除排更 - 按日期範圍', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.red)), Card(color: const Color(0xFFFFEBEE), child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [ SizedBox(width: double.infinity, child: FilledButton.icon(icon: const Icon(Icons.delete_sweep), style: FilledButton.styleFrom(backgroundColor: Colors.red), label: const Text('按日期範圍清除'), onPressed: clearRosterByRange)) ]))), const SizedBox(height: 16), const Text('標準工時 & 承上 & 超時金額', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [ Row(children: [ Expanded(child: TextField(controller: stdCtrl, decoration: const InputDecoration(labelText: '標準工時', suffixText: 'h/週', border: OutlineInputBorder()))), const SizedBox(width: 8), Expanded(child: TextField(controller: carryCtrl, decoration: const InputDecoration(labelText: '承上餘額', border: OutlineInputBorder()))) ]), const SizedBox(height: 10), TextField(controller: otRateCtrl, decoration: const InputDecoration(labelText: '超時金額 /h', prefixText: '\$ ', border: OutlineInputBorder())),
const SizedBox(height: 10),
