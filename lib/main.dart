...
ev=Event(_rosterCalendarId!, title:'${def.code} ${extraType.isNotEmpty?'[$extraType]':''}', description:desc, start:tz.TZDateTime(tz.local,date.year,date.month,date.day), end:tz.TZDateTime(tz.local,date.year,date.month,date.day+1), allDay:true);
}else{
...
ev=Event(_rosterCalendarId!, title:'${def.code} ${def.start}-${def.end}${note.isNotEmpty?' | $note':''}${extraType.isNotEmpty?' | $extraType':''}', description:desc, start:tz.TZDateTime.from(s,tz.local), end:tz.TZDateTime.from(ee,tz.local), allDay:false);
}
...
updateWidget();
if(!silent && mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('已寫入 $_rosterCalendarName 刪$del 加$add 項，去重 [RosterPro]yyyy-MM-dd 全天 allDay:true 含記事和類別')));
...
const SizedBox(height:4),
Text('3. 津貼：${selDef!=null && selDef.hasAllowance?'有 ${selDef.allowance}':'無'} + 單日 \$${rosterExtra[selKey]??0} ${extraType.isNotEmpty?'類別:$extraType':''}',style:const TextStyle(fontSize:12)),
const SizedBox(height:4),
