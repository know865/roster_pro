...
if(region=='香港'){ m['$year-01-01']='元旦'; m['$year-05-01']='勞動節'; m['$year-10-01']='國慶'; m['$year-12-25']='聖誕'; }
return m;
}
...
sp.setString('cName',customName); sp.setString('extraAllowNewV36',jsonEncode(extraAllowances.map((e)=>e.toJson()).toList()));
...
if(!silent&&mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('已寫入 $_rosterCalendarName 刪$del 加$add 去重以 $tag 全天 allDay:true')));
...
for(DateTime d=range.start;!d.isAfter(range.end);d=d.add(const Duration(days:1))){ String k=DateFormat('yyyy-MM-dd').format(d); if(roster.containsKey(k)){ roster.remove(k); count++; } }
...
var backup={'roster':roster,'note':rosterNote,'roOt':rosterOt,'roEx':rosterExtra,'pattern':pattern,'extra':extraAllowances.map((e)=>e.toJson()).toList()};
...
int idx=row*7+col;
DateTime d=days[idx];
bool inM=d.month==focused.month;
...
onLongPress:(){ setState(()=>selectedDay=d); showDetail(d); },
child: Container(
margin: const EdgeInsets.all(2),
decoration: BoxDecoration(color:bg,borderRadius:BorderRadius.circular(12),border:isToday?Border.all(width:2.5,color:todayBorderColor):sel?Border.all(width:2,color:Colors.deepPurple):null),
child: Column(mainAxisAlignment:MainAxisAlignment.center,children:[
Text('${d.day}',style:TextStyle(fontWeight:FontWeight.bold,fontSize:calendarFontSize-1)),
if(code!=null) Container(padding:const EdgeInsets.symmetric(horizontal:4,vertical:1),decoration:BoxDecoration(color:def?.color,borderRadius:BorderRadius.circular(8)),child:Text(code,style:TextStyle(color:Colors.white,fontSize:calendarFontSize-2))),
if(isHol) Container(width:5,height:5,margin:const EdgeInsets.only(top:2),decoration:BoxDecoration(color:holidayDotColor,shape:BoxShape.circle))
])
)
)
)
...
for(int i=1;i<=dim;i++){ DateTime dt=DateTime(focused.year,focused.month,i); String k=DateFormat('yyyy-MM-dd').format(dt); String? c=roster[k]; if(c==null) continue; var d=defs[c]; if(d!=null){ hrs+=d.hours; cnt[c]=(cnt[c]??0)+1; if(d.hasAllowance) allow+=d.allowance; } ot+=(rosterOt[k]??0); allow+=(rosterExtra[k]??0); }
